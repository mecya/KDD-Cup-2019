# -*- coding: utf-8 -*-
"""
Created on Fri Jun 21 17:32:46 2019
...
path = '../data_set_phase2/beijing/'  #or beijing or shenguang
additionaldata_path= '../data_set_phase2/external_data/'
data= pd.read_csv(path+'beijing.csv',parse_dates=['req_time','plan_time'])
...
#读取地铁数据
beijing['metro_station_lng'] = beijing[1]
beijing['metro_station_lat'] = beijing[2]
#始终点到地铁的最近距离        
data['o_nearest_subway_dis', 'd_nearest_subway_dis', 'odis2subway','ddis2subway'] = np.nan
for co in tqdm(all_co['co']):
    #始终点到地铁的最近曼哈顿距离 
    lg, la = co.split(',')
    min_dis = (abs(beijing['metro_station_lng']-float(lg)) + abs(beijing['metro_station_lat']-float(la))).min()
    data.loc[(data['o'] == co), 'o_nearest_subway_dis'] = min_dis
    data.loc[(data['d'] == co), 'd_nearest_subway_dis'] = min_dis
    min_distance = beijing.apply(lambda row:GetDistance(row['metro_station_lng'],row['metro_station_lat'],float(lg),float(la)),axis=1).min()
...
weather_raw = pd.read_csv(additionaldata_path+'Beijing_weather_deal.csv',parse_dates=['datetime'])
...
distance_feature = distance_center_feature + subway_feature + bus_feature + school_feature + distance_feature_1
...
#特征拼接
feature        =  new_feature + origin_num_feature + plan_features + cate_feature + time_feature + distance_feature + xietong_feature + cos_distance + profile + base_feature + rank_feature + od_features + weather_feature
...
data[feature].to_csv(path+'beijing_features.csv',index=False)
